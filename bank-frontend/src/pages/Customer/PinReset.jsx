import React, {useState, useEffect} from 'react';
import { useNavigate } from 'react-router-dom';
import styles from './PinReset.module.css';
import Loading from '../../components/common/Loading';
import { useModal } from '../../context/ModalContext';
const PinReset = () => {
    const navigate = useNavigate();
    const [step, setStep] = useState(1);
    const { openModal } = useModal();
    // Step 1: 본인 확인
    const [name, setName] = useState('');
    const [rrnFirst, setRrnFirst] = useState('');
    const [rrnSecond, setRrnSecond] = useState('');
    const [phone, setPhone] = useState('');
    const [verifyCode, setVerifyCode] = useState('');
    const [isCodeSent, setIsCodeSent] = useState(false);
    const [isVerified, setIsVerified] = useState(false);
    const [isLoading, setIsLoading] = useState(false);

    // Step 2: 기존 핀번호 확인
    const [currentPin, setCurrentPin] = useState('');
    const [pinError, setPinError] = useState('');

    // Step 3: 새로운 핀번호 설정
    const [newPin, setNewPin] = useState('');
    const [confirmNewPin, setConfirmNewPin] = useState('');

    const isValidPhone = /^01[016789]\d{8}$/.test(phone);
    const isFormFilled = name.trim() !== '' && rrnFirst.length === 6 && rrnSecond.length === 7 && isValidPhone;

    const showAlert = (message, callback = null) => {
        openModal({
            message: message,
            onConfirm: callback
        });
    }
    // --- 1단계: 본인 확인 로직 ---
    const handleSendCode = async () => {
        if (!isFormFilled) {
            showAlert('이름, 주민등록번호, 휴대폰 번호를\n모두 올바르게 입력해주세요.');
            return;
        }

        setIsLoading(true);
        try {
            const response = await fetch('/api/sms/send', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone }),
            });
            const data = await response.json();

            // 결과 값에 따른 분기 처리
            switch (data.result) {
                case 'SUCCESS':
                    showAlert('인증번호가 전송되었습니다.');
                    setIsCodeSent(true);
                    break;
                case 'FAILURE':
                    showAlert('인증번호 전송에 실패했습니다.\n번호를 다시 확인해주세요.');
                    break;
                case 'FAILURE_SESSION' :
                    showAlert('세션이 만료되 었습니다. 로그인 후 다시 시도해주세요.');
                    break;
                case 'FAILURE_NOT_SAME_PHONE' :
                    showAlert('전화번호가 일치하지 않습니다.\n다시 시도해주세요.');
                    break;
                default:
                    showAlert('서버 오류가 발생했습니다.\n잠시 후 다시 시도해주세요.');
                    break;
            }
        } catch (error) {
            console.error('전송 에러:', error);
            showAlert('인증번호 전송 중 오류가 발생했습니다.');
        } finally {
            setIsLoading(false);
        }
    };

    const handleVerifyCode = async () => {
        if (verifyCode.length === 0) {
            showAlert('인증번호를 입력해주세요.');
            return;
        }

        setIsLoading(true);
        try {
            const response = await fetch('/api/sms/verify', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone, code: verifyCode }),
            });
            const data = await response.json();

            // 결과 값에 따른 분기 처리
            switch (data.result) {
                case 'SUCCESS':
                    showAlert('인증이 완료되었습니다.', () => {
                        setIsVerified(true);
                        // 인증 완료 후 바로 다음 단계(2단계)로 넘기고 싶다면 아래 주석 해제
                        // setStep(2);
                    });
                    break;
                case 'FAILURE':
                    showAlert('인증번호가 일치하지 않습니다.');
                    break;
                case 'FAILURE_SESSION' :
                    showAlert('세션이 만료되 었습니다. 로그인 후 다시 시도해주세요.');
                    break;
                default:
                    showAlert('인증 확인 중 오류가 발생했습니다.');
                    break;
            }
        } catch (error) {
            console.error('확인 에러:', error);
            showAlert('인증번호 확인 중 오류가 발생했습니다.');
        } finally {
            setIsLoading(false);
        }
    };
    // --- 2단계: 기존 핀번호 확인 로직 ---
    const handleCurrentPinKeyClick = (val) => {
        if (currentPin.length < 6) {
            setCurrentPin(prev => prev + val);
        }
    };

    const handleCurrentPinDelete = () => {
        setCurrentPin(prev => prev.slice(0, -1));
    };

    useEffect(() => {
        const confirmCurrentPin = async () => {
            setIsLoading(true); // 로딩 시작
            try {
                // 요청하신 형식: /api/pin/confirm?pin=123456 (POST 방식)
                const response = await fetch('/api/pin/confirm', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ pin: currentPin }),
                });
                const data = await response.json();

                // 응답 데이터의 결과값에 따른 switch 분기
                switch (data.result) {
                    case 'SUCCESS':
                        // 성공 시 바로 3단계(새 핀번호 입력)로 이동
                        setStep(3);
                        break;

                    case 'FAILURE':
                        showAlert('기존 핀번호가 일치하지 않습니다.');
                        setTimeout(() => {
                            setCurrentPin('');
                            setPinError('');
                        }, 800);
                        break;
                    case 'FAILURE_SESSION' :
                        showAlert('세션이 만료되 었습니다. 로그인 후 다시 시도해주세요.');
                        break;
                    default:
                        showAlert('알 수 없는 오류가 발생했습니다.\n다시 시도해주세요.');
                        setCurrentPin('');
                        break;
                }
            } catch (error) {
                console.error('핀번호 확인 오류:', error);
                showAlert('핀번호 확인 중 오류가 발생했습니다.');
            } finally {
                setIsLoading(false); // 로딩 종료
            }
        };

        // 조건: 2단계(기존 핀 확인)이고, 번호 6자리가 모두 입력되었을 때 실행
        if (step === 2 && currentPin.length === 6) {
            confirmCurrentPin();
        }
    }, [currentPin, step]);

    // --- 3단계: 새로운 핀번호 설정 로직 ---
    const handleNewPinKeyClick = (val) => {
        if (step === 3 && newPin.length < 6) {
            setNewPin(prev => prev + val);
        } else if (step === 3.5 && confirmNewPin.length < 6) {
            setConfirmNewPin(prev => prev + val);
            setPinError('');
        }
    };

    const handleNewPinDelete = () => {
        if (step === 3) setNewPin(prev => prev.slice(0, -1));
        else if (step === 3.5) setConfirmNewPin(prev => prev.slice(0, -1));
    };

    useEffect(() => {
        if (step === 3 && newPin.length === 6) {
            setTimeout(() => setStep(3.5), 300); // 확인 단계로 자동 이동
        }
    }, [newPin, step]);

    useEffect(() => {
        const resetPin = async () => {
            setIsLoading(true); // 로딩 시작
            try {
                // 요청하신 형식: /api/pin/?pin=123456 (PATCH 방식)
                const response = await fetch('/api/pin/', {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ pin: newPin }),
                });
                const data = await response.json();

                // 응답 결과에 따른 switch 분기
                switch (data.result) {
                    case 'SUCCESS':
                        setStep(4); // 성공 시 완료 단계(4)로 이동
                        break;

                    case 'FAILURE':
                    default:
                        showAlert('핀번호 재설정에 실패했습니다.\n다시 시도해주세요.');
                        // 실패 시 입력값 초기화 및 이전 단계(3)로 리셋
                        setTimeout(() => {
                            setNewPin('');
                            setConfirmNewPin('');
                            setPinError('');
                            setStep(3);
                        }, 800);
                        break;
                }
            } catch (error) {
                console.error('핀번호 재설정 에러:', error);
                showAlert('핀번호 재설정 중 오류가 발생했습니다.');
            } finally {
                setIsLoading(false); // 로딩 종료
            }
        };

        // 조건: 3.5단계(확인 입력)이고, 6자리가 모두 입력되었을 때
        if (step === 3.5 && confirmNewPin.length === 6) {
            if (newPin === confirmNewPin) {
                // 새 핀번호와 확인 번호가 일치하면 서버 요청
                resetPin();
            } else {
                // 일치하지 않으면 에러 메시지 표시 후 확인 칸만 초기화
                setPinError('새로운 핀번호가 일치하지 않습니다.');
                setTimeout(() => setConfirmNewPin(''), 800);
            }
        }
    }, [confirmNewPin, newPin, step]);


    // --- 렌더링 로직 ---
    const keypadLayout = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'del'];

    const renderPinDots = (currentLength) => (
        <div className={styles.pinDisplay}>
            {[...Array(6)].map((_, i) => (
                <div key={i} className={`${styles.pinDot} ${i < currentLength ? styles.pinDotFilled : ''}`} />
            ))}
        </div>
    );

    const renderKeypad = (keyHandler, deleteHandler) => (
        <div className={styles.keypad}>
            {keypadLayout.map((key, idx) => {
                if (key === '') return <div key={idx} className={styles.emptyKey} />;
                if (key === 'del') {
                    return (
                        <button key={idx} className={styles.keyBtn} onClick={deleteHandler}>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M21 4H8l-7 8 7 8h13a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2z"></path>
                                <line x1="18" y1="9" x2="12" y2="15"></line>
                                <line x1="12" y1="9" x2="18" y2="15"></line>
                            </svg>
                        </button>
                    );
                }
                return (
                    <button key={idx} className={styles.keyBtn} onClick={() => keyHandler(key)}>
                        {key}
                    </button>
                );
            })}
        </div>
    );

    return (
        <div className={styles.pageContainer}>
            {isLoading && <Loading message="처리 중..." />}
            <div className={styles.contentBox}>
                <div className={styles.sidebar}>
                    <h2 className={styles.logoTitle}>BANKSCOPE</h2>
                    <h3 className={styles.pageTitle}>핀 번호 재설정</h3>
                    <ul className={styles.stepList}>
                        {[
                            { id: 1, text: '본인 확인' },
                            { id: 2, text: '기존 핀번호 확인' },
                            { id: 3, text: '핀번호 재설정' },
                            { id: 4, text: '재설정 완료' }
                        ].map((s) => (
                            <li key={s.id} className={step >= s.id ? styles.activeStep : styles.inactiveStep}>
                                <div className={styles.stepCircle}>{s.id}</div>
                                <span>{s.text}</span>
                            </li>
                        ))}
                    </ul>
                </div>

                <div className={styles.mainContent}>
                    {step === 1 && (
                        <div className={styles.stepContainer}>
                            <h3 className={styles.sectionTitle}>본인 인증 절차가 필요합니다.</h3>
                            <p className={styles.sectionSubtitle}>SMS 인증</p>
                            <div className={styles.formGroup}>
                                <label>이름</label>
                                <input type="text" placeholder="이름 입력" className={styles.input} value={name} onChange={(e) => setName(e.target.value)} />
                            </div>
                            <div className={styles.formGroup}>
                                <label>주민등록번호</label>
                                <div className={styles.rrnWrapper}>
                                    <input type="text" maxLength={6} placeholder="앞 6자리" className={styles.input} value={rrnFirst} onChange={(e) => setRrnFirst(e.target.value.replace(/[^0-9]/g, ''))} />
                                    <span>-</span>
                                    <input type="password" maxLength={7} placeholder="뒤 7자리" className={styles.input} value={rrnSecond} onChange={(e) => setRrnSecond(e.target.value.replace(/[^0-9]/g, ''))} />
                                </div>
                            </div>
                            <div className={styles.formGroup}>
                                <label>휴대폰 번호</label>
                                <div className={styles.phoneWrapper}>
                                    <input type="text" placeholder="- 없이 010부터 입력" className={styles.input} value={phone} maxLength={11} onChange={(e) => setPhone(e.target.value.replace(/[^0-9]/g, ''))} />
                                    <button className={`${styles.actionBtn} ${!isFormFilled ? styles.disabledActionBtn : ''}`} onClick={handleSendCode} disabled={!isFormFilled || isVerified || isLoading}>
                                        {isCodeSent ? '재전송' : '인증번호 전송'}
                                    </button>
                                </div>
                                {phone.length > 0 && !isValidPhone && <p style={{ color: '#e74c3c', fontSize: '12px', marginTop: '5px' }}>올바른 휴대폰 번호 형식이 아닙니다.</p>}
                            </div>
                            {isCodeSent && (
                                <div className={styles.formGroup}>
                                    <label>인증번호</label>
                                    <div className={styles.verifyWrapper}>
                                        <input type="text" placeholder="인증번호 입력" className={styles.input} value={verifyCode} onChange={(e) => setVerifyCode(e.target.value.replace(/[^0-9]/g, ''))} disabled={isVerified} />
                                        <button className={`${styles.actionBtn} ${isVerified ? styles.verifiedBtn : ''}`} onClick={handleVerifyCode} disabled={isVerified || verifyCode.length === 0 || isLoading}>
                                            {isVerified ? '인증완료' : '인증 확인'}
                                        </button>
                                    </div>
                                </div>
                            )}
                            <button className={isVerified ? styles.nextBtnActive : styles.nextBtnDisabled} onClick={() => setStep(2)} disabled={!isVerified}>
                                다음 단계로
                            </button>
                        </div>
                    )}

                    {step === 2 && (
                        <div className={styles.stepContainer}>
                            <h3 className={styles.sectionTitle}>기존 핀번호를 입력해주세요.</h3>
                            {pinError && <p className={styles.errorMessage}>{pinError}</p>}
                            {renderPinDots(currentPin.length)}
                            {renderKeypad(handleCurrentPinKeyClick, handleCurrentPinDelete)}
                        </div>
                    )}

                    {(step === 3 || step === 3.5) && (
                        <div className={styles.stepContainer}>
                            <h3 className={styles.sectionTitle}>
                                {step === 3 ? '새로운 핀번호를 입력해주세요.' : '핀번호를 한번 더 입력해주세요.'}
                            </h3>
                            {pinError && <p className={styles.errorMessage}>{pinError}</p>}
                            {renderPinDots(step === 3 ? newPin.length : confirmNewPin.length)}
                            {renderKeypad(handleNewPinKeyClick, handleNewPinDelete)}
                        </div>
                    )}

                    {step === 4 && (
                        <div className={styles.completeContainer}>
                            <div className={styles.successIcon}>
                                <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#4A9C82" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                                    <polyline points="22 4 12 14.01 9 11.01"></polyline>
                                </svg>
                            </div>
                            <h3 className={styles.sectionTitle}>핀 번호 재설정 완료</h3>
                            <p className={styles.sectionSubtitle} style={{marginBottom: '50px'}}>
                                새로운 핀 번호가 성공적으로 등록되었습니다.
                            </p>
                            <button className={styles.nextBtnActive} onClick={() => navigate('/My')}>
                                마이페이지로 돌아가기
                            </button>
                        </div>
                    )}
                </div>
            </div>

        </div>
    );
};

export default PinReset;
