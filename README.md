...
- **빈 DB**: 백엔드를 처음 실행하면 Flyway가 V1~V6를 적용하여 스키마+데이터를 자동 구성한다. (`bank` 스키마가 없으면 자동 생성)
...
- **주의**: 적용 완료된 마이그레이션 파일을 수정한 뒤에는 기존 DB의 Flyway 체크섬과 맞지 않을 수 있다. 이 경우 기존 제출용 더미 DB를 비우고 V1~V6를 처음부터 다시 적용해야 한다.

> V1\~V3/V5/V6는 SQL 기반 Flyway 마이그레이션이고, V4는 Java 기반 Flyway 마이그레이션이다. V1\~V3 SQL만 수동 실행하면 주민번호 보호 컬럼과 검색 인덱스가 완성되지 않으므로, 최종 DB 구성은 백엔드 기동을 통해 Flyway가 V6까지 적용하게 해야 한다. V5의 `DCG` prefix 이력은 관리자 혼잡도 UI 시연용 합성 완료 업무이며, AI 업무 예측 모델 학습에서는 제외된다.
...
직원 확인과 접수 스냅샷을 갖춘 완료 기록을 검토 후 포함하려면 `python RF.py --include-confirmed-db`를 명시한다. 과거 완료 기록·AI 예측만 있는 기록은 자동 병합하지 않는다. 재학습 모델을 반영하려면 AI 서버를 재시작한다.
...
- 대기: **업무 수락 / 업무 이관**. 수락하려면 근무중 상태여야 한다. 배정된 담당자는 직급을 다시 검사하지 않고 해당 업무를 수락·종료할 수 있다. 직원의 직급 자체가 변경되지는 않는다.
- 상담 중: **업무 이관 / 업무 종료**. 수락 취소는 더보기 메뉴에서 실행하며 거래를 취소하지 않는다.
- 이관: 실제 업무·사유를 입력하고 업무 권한과 근무 상태를 충족하는 직원을 선택한다. 기존 번호표·최초 접수 시각을 유지하고 이관 이력을 저장한다.
- 종료: 실제 처리 업무를 확인한다. 최초 AI 예상은 별도 필드에 보존한다.
- 배포 순서: 백엔드를 시작해 V6를 적용한 후, 변경된 프론트엔드와 AI 서버를 함께 반영한다. 기존 데이터는 유지된다.
